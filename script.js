...
const assetsReady=Promise.all([txt('assets/live-photo.txt'),txt('assets/live-music-00.txt'),txt('assets/live-music-01.txt'),txt('assets/live-music-02.txt'),txt('assets/live-music-03.txt')]).then(([photo,m0,m1,m2,m3])=>{$('heroPhoto').src='data:image/avif;base64,'+photo;audio.src='data:audio/ogg;base64,'+[m0,m1,m2,m3].join('');audio.load();carriage.disabled=false;carriage.removeAttribute('aria-busy')}).catch(err=>{console.error(err);carriage.disabled=false;carriage.removeAttribute('aria-busy')});
...
carriage.addEventListener('click',()=>{document.body.classList.remove('entry-locked');document.body.classList.add('invitation-open');tryPlay();setTimeout(()=>document.querySelector('main')?.scrollIntoView({block:'start'}),100)});
